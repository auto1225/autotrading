...
나중에 다시 실행하거나 새 PC에서 복구할 때는 [재실행/복구 런북](docs/restart-runbook.md)을 먼저 따라가세요. 설치, `.env` 준비, 서버 실행, 1000 USDT 모의계좌 리셋, 알렉스/매억남 모드, 수동 API, 24시간 점검 에이전시, 테스트와 커밋 절차를 한 문서에 정리했습니다.
